...
class PathAgent {
    ...
    calculate() {
        ...
        var nodesToConsiderById = new Map();
        var nodesAlreadyConsidered = new Map();
        while (nodesToConsider.length > 0) {
            var nodeToConsider = nodesToConsider[0];
            if (nodeToConsider.cellPos.equals(this.goalPos)) {
                this.nodes = this.calculate_3_BuildListOfNodesFromStartToGoal(nodeToConsider);
                break;
            }
            else {
                ...
                this.calculate_2_AddNeighborsToListOfNodesToConsider(nodeToConsider, nodesToConsider, nodesToConsiderById, nodesAlreadyConsidered);
            }
        }
    }
    calculate_1_InitializeListOfNodesToConsider() {
        ...
        displacementFromStartToGoal.overwriteWith(this.goalPos).subtract(this.startPos);
        var costFromStartToGoalEstimated = displacementFromStartToGoal.absolute().sumOfDimensions();
        var startNode = new PathNode(this.startPos, // cellPos
        0, // costFromStart
        costFromStartToGoalEstimated, null // prev
        );
        nodesToConsider.push(startNode);
        return nodesToConsider;
    }
    calculate_2_AddNeighborsToListOfNodesToConsider(nodeToFindNeighborsOf, nodesToConsider, nodesToConsiderById, nodesAlreadyConsidered) {
        ...
        for (var n = 0; n < nodesNeighboring.length; n++) {
            ...
            var hasNodeNeighborNotYetBeenSeen = (nodesAlreadyConsidered.has(nodeNeighborID)
                && nodesToConsiderById.has(nodeNeighborID));
            if (hasNodeNeighborNotYetBeenSeen) {
                ArrayHelper2.insertElementSortedByKeyName(nodesToConsider, nodeNeighbor, "costToGoalEstimated");
