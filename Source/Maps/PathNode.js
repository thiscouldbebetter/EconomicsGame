...
class PathNode {
    ...
    id(mapSizeInCells) {
        var nodeToConsiderIndex = this.cellPos.y
            * mapSizeInCells.y
            + this.cellPos.x;
    ...
    neighbors(path) {
        ...
        for (var i = 0; i < neighborOffsets.length; i++) {
            ...
            neighborPos.overwriteWith(this.cellPos).add(neighborOffset);
            if (mapBoundsInCellsMinusOnes.containsPoint(neighborPos)) {
                ...
                var neighborCostFromStart = this.costFromStart
                    + neighborCostToTraverse;
                var neighborCostToGoalEstimated = neighborCostToTraverse
                    + costToGoalTemp.overwriteWith(path.goalPos).subtract(this.cellPos).absolute().sumOfDimensions();
                var neighborNode = new PathNode(neighborPos.clone(), neighborCostFromStart, neighborCostToGoalEstimated, this // prev
                );
